...
const Role = () => {
  ...
  const getList = async (parameter = {}) => {
    ...
    const res = await request("role" + param, "GET");
    setTimeout(() => {
      setLoadin(false);
    }, 300);
    setList(res.list);
  ...
  const onDelete = async (rows) => {
    ...
    const res = await request("role", "delete", param);
    if (res) {
      getList();
  ...
  return (
    <MainPage loading={loading}>
      <div className="mb-2">
        <div className="text-5xl text-center mb-5">roles</div>
        <div className="flex justify-between ">
          <div className="flex ">
            <Space>
              <div className="text-md w-full lg:text-2xl ">
                Total roles: {totalRecord}
              </div>
      ...
      >
        <Form  layout="vertical" form={form} name="control-hooks" onFinish={onFinish}>
          <Divider />
            ...
            <Form.Item name="code" label="Code Name"  rules={[
                  {
                    required: true,
                  },
                ]}>
